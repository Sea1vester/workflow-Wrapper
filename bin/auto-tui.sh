...
draw_frame() {
  local frame="$1"
  local outcome phase iter max last_tests message pulse doing
  ...
  if [ -z "$doing" ] && [ "$phase" = "agent" ]; then
    if [ -f "$AUTO_DIR/doing" ]; then
      doing="$(cat "$AUTO_DIR/doing")"
    else
      doing="Agent is naming this try"
    fi
  fi
