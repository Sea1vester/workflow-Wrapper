...
function fill(template: string, args: Record<string, string | undefined>): string {
...
export const WFW_PROMPTS: WfwPromptDefinition[] = [
  ...
  },
  {
    ...
    template: (args) =>
      fill(
        `Run workflowWrapper start for feature: {{feature}}
...
Report the worktree path and suggest wfw_plan next.`,
        args,
      ),
  },
  {
...
That posts your reply in Lavish and polls again - wfw auto-resumes listen if the harness interrupts the poll.`,
  ...
  },
  {
    ...
    template: (args) =>
      fill(
        `Run workflowWrapper prompt with: {{prompt}}
...
After feedback, use wfw_plan with agent_reply to reply in the browser and poll again.`,
  ...
  },
  {
    ...
    template: (args) =>
      fill(
        `Run workflowWrapper agent for feature: {{feature}}
...
Detects claude, opencode, agy, gemini, cursor, or agent on PATH (override: WFW_AGENT_CLI).`,
  ...
  },
  {
    ...
    template: (args) =>
      fill(
        `Run workflowWrapper auto with objective: {{objective}}

Use the wfw_auto MCP tool with objective="{{objective}}".

Loop: one agent change per iteration, keep or revert by tests.
Stops when .wfw/auto/DONE exists and tests pass (default cap 12 iterations).`,
        args,
      ),
  },
  {
